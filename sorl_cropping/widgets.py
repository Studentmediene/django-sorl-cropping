...
def thumbnail(image_path):
    thumb = get_thumbnail(image_path, getattr(settings, 'SORL_CROPPING_THUMB_SIZE', '300x300'))
    return thumb.url


def get_attrs(image, name):
    width, height = default.engine.get_image_size(default.engine.get_image(image))
    try:
        return {
            'class': "crop-thumb",
